...
def merge_with_defaults(fetched):
    ...
    or holds a company generated by an older version of the code (missing newer fields)."""
    ...
    # Backfill a company object created by an older schema version (missing the
    # newer financial display fields) by regenerating it deterministically from
    # the same seed — keeps round_defaulted and the loan terms identical.
    company = base.get("company")
    if isinstance(company, dict) and "turnover" not in company and base.get("company_seed") is not None:
        base["company"] = generate_company(base["company_seed"])
    return base
